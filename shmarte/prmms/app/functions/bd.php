...
function crearBd(){
  ...
  if (!$con) {
    $con = newDataBase();
    $con->exec("drop database pinkladyseries");

    /* crear la base de datos */
    $sql = "create database pinkladyseries";
    $res = $con->exec($sql);
    if(!$res){
    ...
    $con = dbConection();
    if($con===false){
      echo "<p class='text-danger'>Conexión con BD pinkladyseries no establecida</p>";
      echo "<p class='text-danger'>".$con->errorInfo()[2]."</p>";
    }
    ...
    $res = $con->exec($sql);
    if($res===false){
      echo "<p class='text-danger'>No se ha podido crear la tabla genero.</p>";
      echo "<p class='text-danger'>".$con->errorInfo()[2]."</p>";
    }
    ...
    $res = $con->exec($sql);
    if($res===false){
      echo "<p class='text-danger'>No se ha podido crear la tabla series.</p>";
      echo "<p class='text-danger'>".$con->errorInfo()[2]."</p>";
    }
