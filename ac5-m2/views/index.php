...
<body>
  ...
  <main>
    <nav class="navbar navbar-default">
      <div class="container">
        <ul class="nav navbar-nav">
          <li class="active"><a href='<?php echo $data['urlbase'];?>'>Home</a></li>
    ...
    <div class="container">
      <?php
      $series = array_keys($data['series']);
      echo "<table class='table'>";
      $array = count($series);
      if ($array>=3) {
        for ($i=$array-3; $i<$array; $i++) {
          $s = $data['series'][$series[$i]];
          echo "<tr><th colspan='2'>{$s['titulo']}</th>";
          echo "</tr><tr><td><img src='{$s['imagen']}'></a></td>";
          echo "<td>{$s['sinopsis']}</td></tr>";
        }
        echo "</table>";
      } else {
        foreach ($data['series'] as $value) {
          echo "<tr><th colspan='2'>{$value['titulo']}</th><th></th>";
